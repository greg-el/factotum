...
fn get_task_snapshot_clones() {
    ...
    assert!(
        &snapshot[0].run_started.unwrap() > &UTC::now().checked_sub(Duration::seconds(60)).unwrap()
    );
...
fn execute_sends_started_msg() {
    ...
    match expected_starting.execution_state {
        ExecutionState::Started => {
            assert!(expected_starting
                .task_snapshot
                .iter()
                .all(|t| t.state == State::Waiting));
            assert_eq!(
                expected_starting.task_snapshot.len(),
...
fn execute_sends_running_messages() {
    ...
    let total_expected_task_updates = count_starting_msg
        + count_done_msg
...
fn execute_sends_updates_for_skipped_fail() {
    ...
    let is_apple_failing = match final_task_update
        .task_snapshot
        .iter()
        .find(|t| t.name == "apple")
        .unwrap()
    ...
    let is_turnip_failing = match final_task_update
        .task_snapshot
        .iter()
        .find(|t| t.name == "turnip")
        .unwrap()
        .state
    {
        State::Failed(_) => true,
    ...
    let is_egg_skipped = match final_task_update
        .task_snapshot
        .iter()
        .find(|t| t.name == "egg")
        .unwrap()
    ...
    let expected_task_transition = vec![
        TaskTransition::new(
            "turnip",
            State::Running,
            State::Failed(
        ...
        TaskTransition::new(
            ...
            State::Skipped(
                "the task 'apple' failed, the task 'turnip' \
                                                 failed"
                    .to_string(),
            ),
        ),
...
fn execute_sends_failed_skipped_messages() {
    ...
    let (job_updates, task_updates): (Vec<ExecutionUpdate>, Vec<ExecutionUpdate>) =
        recv_msg.into_iter().partition(|m| match m.transition {
    ...
    assert!(job_updates[0]
        .task_snapshot
        .iter()
        .all(|t| t.state == State::Waiting));
    assert_eq!(
        job_updates[0].transition,
    ...
    let task_apple_failed = match job_updates[2]
        .task_snapshot
        .iter()
        .find(|t| t.name == "apple")
        .unwrap()
    ...
    let task_turnip_skipped = match job_updates[2]
        .task_snapshot
        .iter()
        .find(|t| t.name == "turnip")
        .unwrap()
    ...
    let task_turnip_skipped_task_update = match task_updates[1]
        .task_snapshot
        .iter()
        .find(|t| t.name == "turnip")
        .unwrap()
...
fn execute_sends_updates_for_skipped_noop() {
    ...
    let is_apple_noop = match final_task_update
        .task_snapshot
        .iter()
        .find(|t| t.name == "apple")
        .unwrap()
    ...
    let is_turnip_noop = match final_task_update
        .task_snapshot
        .iter()
        .find(|t| t.name == "turnip")
        .unwrap()
    ...
    let expected_task_transition = vec![
        TaskTransition::new("turnip", State::Running, State::SuccessNoop),
        TaskTransition::new(
            "egg",
            State::Skipped("the task 'apple' requested early termination".to_string()),
            State::Skipped(
                "the task 'apple' requested early termination, \
...
fn execute_sends_noop_skipped_messages() {
    ...
    assert_eq!(job_updates[2].execution_state, ExecutionState::Finished);
    let task_apple_noop = match job_updates[2]
        .task_snapshot
        .iter()
        .find(|t| t.name == "apple")
        .unwrap()
    ...
    let task_egg_skipped = match job_updates[2]
        .task_snapshot
        .iter()
        .find(|t| t.name == "egg")
        .unwrap()
    ...
    let task_apple_noop_task_update = match task_updates[1]
        .task_snapshot
        .iter()
        .find(|t| t.name == "apple")
        .unwrap()
    ...
    let task_egg_skipped_task_update = match job_updates[2]
        .task_snapshot
        .iter()
        .find(|t| t.name == "egg")
        .unwrap()
